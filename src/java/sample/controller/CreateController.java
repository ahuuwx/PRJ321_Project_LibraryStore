...
public class CreateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (bookID == null || !bookID.matches("^B\\d{2}$")) {
                errorBook.setBookIDError("Book ID is not null and format: BXX");
                check = false;
            }
            ...
            if (price == "" || !price.matches("^\\d+(\\.\\d+)?")) {
                errorBook.setPriceError("Price is not null and is a number");
                check = false;
            }
            if (bookType == "") {
            ...
            BookDAO dao = new BookDAO();
             existed=dao.checkExistedID(bookID);
            if(existed==true){
                errorBook.setBookIDError("Duplicated ID");
                check=false;
            }
            if (check) {
